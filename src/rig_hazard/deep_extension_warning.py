...
def build_extension_warning_report(
    ...
    return "\n".join(
        [
            "# Deep RIG-Hazard Extension: Full-Timeline Event Evaluation",
            "",
            f"Generated at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            "",
            "## Frozen causal monthly budget",
            "",
            f"All models use the same {budget_config.candidate_quantile:.2f} rolling quantile over the previous 30 days and a hard limit of {budget_config.monthly_budget_hours:.1f} total alert hours per station-month.",
            "",
            ...
            "",
            "Each model uses 2023 development labels to select a probability threshold that matches 10 aggregate false-alarm hours per station-month. This is a capability comparison, not a deployment threshold.",
            "",
            ...
            "",
            "### Threshold and monthly distribution",
            "",
            ...
            "",
            "All extension selection and comparison stops at the 2023 development year. Data from 2024 do not influence recurrence-history, negative-sampling, barrier-regularization, or model selection; publication-level confirmation still requires a new year or external region.",
        ]
    ) + "\n"
